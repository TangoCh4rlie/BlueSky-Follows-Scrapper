...
```bash
pnpm run start "<blueSkyUsername>" "<limitUser>" "<limitApi>"
```

blueSkyUsername` is the `handle` field returned by the api. Example: `tangoch4rlie.bsky.social`
...
`limitApi` is just a parameter to limit the number of api call 

### Example 

To process 10 users
```bash
pnpm run start tangoch4rlie.bsky.social 10 
```